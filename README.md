...
## Firmware notes
- The firmware sends MAVLink `BATTERY_STATUS` frames to represent the external
  current source so it does not overwrite the vehicle's primary battery when
  you choose a unique `MAV_SYSTEM_ID` and `MAV_BATTERY_ID_EXTERNAL`.
...
---
If you want I can push these local changes to the remote `main` branch (preserving the
LICENSE). Reply `push` to have me run the safe force-push steps for you.
